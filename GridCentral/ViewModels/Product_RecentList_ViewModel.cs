...
{
   public class Product_RecentList_ViewModel : Base_ViewModel
    {
        #region Bind Property
        bool _noItems;
        ObservableCollection<mSearchProduct> _RecentList = new ObservableCollection<mSearchProduct>();
        ...
        public ObservableCollection<mSearchProduct> RecentList
        {
            get { return _RecentList; }
            set { _RecentList = value; OnPropertyChanged("RecentList"); OnPropertyChanged("RevRecentList"); }
        }

        public ObservableCollection<mSearchProduct> RevRecentList
        {
            get { return new ObservableCollection<mSearchProduct>(RecentList.Reverse()); }
        }
        #endregion
        public Product_RecentList_ViewModel()
        {
            GetRecents(0, 10, false);
        }
        public async void GetRecents(int len, int amount, bool addon)
        {
            if (IsBusy) return;
            ...
            try
            {
                ...
                if (addon)
                {
                    ...
                    RecentList.AddRange(formData(result));
                }
                else
                {
                    ProductList = result;
                    RecentList = formData(result);
                }
               // OnPropertyChanged("RevRecentList");
            }
            catch (Exception ex)
            {
